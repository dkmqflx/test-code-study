beforeEach(() => {
  ...
  // 첫번째, 두번째 상품을 장바구니에 담고
  // 강의에서는 편의상 인덱스를 사용
  // 실제 테스트: 특정 id나 이름을 가진 상품을 기준으로 진행 -> 상품 목록의 순서 변경에 영향 없음
...
describe('배송 정보', () => {
  ...
  it('할인 쿠폰을 선택하지 않은 경우 "없음"이 노출되며, 특정 할인 쿠폰을 선택하면 해당 쿠폰의 이름("가입 기념! $5 할인 쿠폰")이 노출된다', () => {
    ...
    // 3. 배송 정보 테이블에 가입 기념! $5 할인 쿠폰이 노출되는지 단언
    cy.get('@shippingList')
      .eq(2)
...
describe('구매 물품', () => {
  beforeEach(() => {
    cy.findAllByRole('table').eq(1).findAllByRole('row').as('itemList'); // 반복되는 요소에 대해 alias 설정
  });
...
describe('결제 정보', () => {
  beforeEach(() => {
    // 반복되는 요소에 대해 alias 설정
    // alias는 테스트 실행 전에 해당 블록 내에서 항상 초기화됨
    // -> 기존에 설정한 shippingList alias는 배송 정보 describe 블록 내에서만 사용 가능
    cy.findAllByRole('table').eq(0).findAllByRole('row').as('shippingList');
...
// 구매하기 API 응답을 스터빙 하여 구매 프로세스 테스트 진행
